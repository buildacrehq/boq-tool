...
export function calculateFullBOQ(project, prices) {
  ...
  floorsData.forEach((floor, index) => {
    ...
    // Flooring Labour
    add(stageName, `Flooring Labour — ${floor.name}`, 'Per Sqft', floorArea, flooringLabourRate, floorArea * flooringLabourRate)
    add(stageName, `Epoxy Grouting — ${floor.name}`, 'Per Sqft', floorArea, epoxyGroutingRate, floorArea * epoxyGroutingRate)
  })
  ...
  floorsData.forEach(floor => {
    ...
    if (floor.poojaRoom) add(stageName, 'Pooja Room Door', 'Per Door', 1, poojaRoomDoorRate, poojaRoomDoorRate)
  })
