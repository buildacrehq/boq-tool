...
export function calculateFullBOQ(project, prices, boqData = {}, vehicleData = {}) {
  ...
  // Terrace Room (if selected)
  if (project.terrace_room_type) {
    if (project.terrace_room_type === 'preset_basic') {
      const cost = parseFloat(prices['Terrace Room Basic']) || 70000
      add('Terrace', 'Terrace Room — 100 sqft (all-in)', 'Lumpsum', 1, cost, cost)
    } else if (project.terrace_room_type === 'preset_bath') {
      const cost = parseFloat(prices['Terrace Room With Bathroom']) || 200000
      add('Terrace', 'Terrace Room with Bathroom — 100 sqft (all-in)', 'Lumpsum', 1, cost, cost)
    } else if (project.terrace_room_type === 'custom') {
      const tSqft = parseFloat(project.terrace_room_sqft) || 0
      const tRate = parseFloat(project.terrace_room_rate) || parseFloat(prices['Terrace Room Rate']) || 2000
      if (tSqft > 0) {
        const tTotal = Math.ceil(tSqft * tRate)
        add('Terrace', `Terrace Room — ${tSqft} sqft × ₹${tRate}/sqft`, 'Lumpsum', 1, tTotal, tTotal)
      }
    }
  }
