...
export function calculateFullBOQ(project, prices, boqData = {}, vehicleData = {}) {
  ...
  // Terrace Room — formula: (sqft × rate) + (washrooms × washroom cost)
  if (floorsData[0]?.terraceRoomEnabled) {
    ...
    if (tSqft > 0) {
      ...
      const tTotal = Math.ceil(tSqft * tRate + tWash * tWashCost)
      const label = tWash > 0
        ? `Terrace Room — ${tSqft} sqft + ${tWash} washroom${tWash > 1 ? 's' : ''}`
        : `Terrace Room — ${tSqft} sqft`
