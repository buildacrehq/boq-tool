...
function getValue(row, inputArea, prices) {
  ...
  if (row.divisible) {
    // Use lower bin as base so quantity always increases with area
...
export function calculateFullBOQ(project, prices, boqData = {}, vehicleData = {}) {
  ...
  floorsData.forEach((floor, index) => {
    ...
    // Masonry (non-divisible — use upper bin)
    if (!isParking) {
    ...
    // Cement (divisible — scale from lower bin so quantity always increases with area)
    ...
    // M Sand (non-divisible vehicle — use upper bin)
    ...
    // 20mm Aggregate (non-divisible vehicle — use upper bin)
