...
export function calculateFullBOQ(project, prices, boqData = {}, vehicleData = {}) {
  ...
  addVehicle('Terrace', 'M Sand', 'M Sand', terraceMsandRow[terraceSizeKey], inputArea)
  ...
  add('Terrace', 'Electrical (Slab & Wall)', 'Lumpsum', 1, terraceElecRow[terraceSizeKey], terraceElecRow[terraceSizeKey])

  const terraceMiscRow = boqData['terrace_misc'] || BOQ_TABLE.terrace_misc
  add('Terrace', 'Misc (Oil, Cover, Chape)', 'Lumpsum', 1, terraceMiscRow[terraceSizeKey], terraceMiscRow[terraceSizeKey])
  ...
  addVehicle('Screed Concrete', 'P Sand', 'P Sand', screedPsandRow[terraceSizeKey], inputArea)

  const screed12mmRow = vehicleData['screed_12mm'] || BOQ_TABLE.screed_12mm
  addVehicle('Screed Concrete', '12mm Aggregate', '12mm Aggregate', screed12mmRow[terraceSizeKey], inputArea)

  const screedMiscRow = boqData['screed_misc'] || BOQ_TABLE.screed_misc
  add('Screed Concrete', 'Misc', 'Lumpsum', 1, screedMiscRow[terraceSizeKey], screedMiscRow[terraceSizeKey])
  ...
  if (project.has_compound_wall) {
    const cwBlocksQty = boqData['cw_blocks']?.[siteKey] ?? BOQ_TABLE.cw_blocks[siteKey]
    const cwCementQty = boqData['cw_cement']?.[siteKey] ?? BOQ_TABLE.cw_cement[siteKey]
    const cwLabour    = boqData['cw_labour']?.[siteKey] ?? BOQ_TABLE.cw_labour[siteKey]
    const cwPlaster   = boqData['cw_plaster']?.[siteKey] ?? BOQ_TABLE.cw_plaster[siteKey]
    if (useBlocks) {
  ...
  if (floorsData[0]?.parapetEnabled !== false) {
    const parapetBlocksQty = boqData['parapet_blocks']?.[siteKey] ?? BOQ_TABLE.parapet_blocks[siteKey]
    const parapetCementQty = boqData['parapet_cement']?.[siteKey] ?? BOQ_TABLE.parapet_cement[siteKey]
